...
CSysModule *Sys_LoadModule( const char *pModuleName )
{
        ...
        HMODULE hDLL = NULL;
        // if a full path was not passed in, try locations adjacent to the tool first
#ifndef _XBOX
        if ( !Q_IsAbsolutePath(pModuleName) )
        {
                ...
                if ( exeLen > 0 && exeLen < sizeof( exePath ) )
                {
                        ...
                        Q_snprintf( szAbsoluteModuleName, sizeof( szAbsoluteModuleName ), "%s/%s", exePath, pModuleName );
                        hDLL = Sys_LoadLibrary( szAbsoluteModuleName );
                        if ( !hDLL )
                        {
                                Q_snprintf( szAbsoluteModuleName, sizeof( szAbsoluteModuleName ), "%s/bin/%s", exePath, pModuleName );
                                hDLL = Sys_LoadLibrary( szAbsoluteModuleName );
                        }
                }
#endif

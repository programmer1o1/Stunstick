...
bool KeyValues::LoadFromFile( IBaseFileSystem *filesystem, const char *resourceName, const char *pathID )
{
	...
	// Load file into a null-terminated buffer. Avoid IFileSystem-only calls so that
	// tools can operate with a minimal IBaseFileSystem implementation.
	const int fileSize = static_cast<int>( filesystem->Size( f ) );
	if ( fileSize <= 0 )
	{
		filesystem->Close( f );
		return false;
	...
	const int bytesRead = filesystem->Read( buffer, fileSize, f );
	buffer[( bytesRead > 0 ) ? bytesRead : 0] = 0;
	filesystem->Close( f );

	const bool retOK = LoadFromBuffer( resourceName, buffer, filesystem );
	delete[] buffer;
	return retOK;
}
...
bool KeyValues::WriteAsBinary( CUtlBuffer &buffer )
{
	...
	for ( KeyValues *dat = this; dat != NULL; dat = dat->m_pPeer )
	{
		...
		case TYPE_PTR:
			{
			uintptr_t ptrVal = reinterpret_cast<uintptr_t>( dat->m_pValue );
			buffer.PutUnsignedInt( static_cast<unsigned int>( ptrVal & 0xFFFFFFFFu ) );
			}

...
#if !defined(_WIN32)
// Fallback scalar implementations for non-Windows builds.
...
float _3DNow_InvRSquared(const float* v)
{
	...
	return ( lenSq > 0.0f ) ? 1.0f / lenSq : 0.0f;
}

#elif !defined(COMPILER_MSVC64) && !defined(LINUX)
...
float _3DNow_Sqrt(float x)
{
	...
	(
 	...
 	__asm __volatile__( "femms" );
#else
        // 3DNow not supported on this platform; fall back to scalar math
        float lenSq = v[0]*v[0] + v[1]*v[1] + v[2]*v[2];
        if ( lenSq > 0.0f )
        {
            float invLen = 1.0f / sqrtf( lenSq );

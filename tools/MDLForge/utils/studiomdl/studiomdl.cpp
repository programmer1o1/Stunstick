...
void *kalloc( int num, int size )
{
	...
	ptr = reinterpret_cast<byte *>(
		( reinterpret_cast<std::uintptr_t>( ptr ) + 511u ) & ~std::uintptr_t( 511u ) );
	return ptr;
}
...
static void SkipOptionalBracedBlock()
{
	if ( !GetToken( true ) )
		return;
...
int main (int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		{
			// Skip the gamedir parameter so it isn't mistaken as the qc file.
			if ( i + 1 >= argc )
				UsageAndExit();
			++i;
			continue;
...
//					break;
	...
	//
	ClearModel();
	Q_StripExtension( argv[qcArgIndex], outname, sizeof( outname ) );

...
#undef ALIGN64
#undef ALIGN512
#define ALIGN4( a ) a = (byte *)( ( (uintptr_t)(a) + 3u ) & ~(uintptr_t)3u )
#define ALIGN16( a ) a = (byte *)( ( (uintptr_t)(a) + 15u ) & ~(uintptr_t)15u )
#define ALIGN32( a ) a = (byte *)( ( (uintptr_t)(a) + 31u ) & ~(uintptr_t)31u )
#define ALIGN64( a ) a = (byte *)( ( (uintptr_t)(a) + 63u ) & ~(uintptr_t)63u )
#define ALIGN512( a ) a = (byte *)( ( (uintptr_t)(a) + 511u ) & ~(uintptr_t)511u )
...
//				j, i, &pbone[j] );
		...
		if (!g_quiet)
		{
			printf( "vertices   %7td bytes (%d vertices)\n", pData - cur, pLodDataSrc->numvertices );
		}
	}
	...
	for (i = 0; i < g_nummodelsbeforeLOD; i++) 
	{
		...
		if (!g_quiet)
		{
			printf( "tangents   %7td bytes (%d vertices)\n", pData - cur, pLodDataSrc->numvertices );
		}
	}

	if (!g_quiet)
	{
		printf( "total      %7td bytes\n", pData - pStart );
	}
...
static void WriteModel( studiohdr_t *phdr )
{
	...
	uintptr_t externalVertexIndex   = 0;
	uintptr_t externalTangentsIndex = 0;
	...
	for (i = 0; i < g_nummodelsbeforeLOD; i++) 
	{
		...
		for (m = 0; m < pmodel[i].nummeshes; m++)
		{
			...
			externalVertexIndex = ( externalVertexIndex + 15u ) & ~uintptr_t( 15u );
			pmodel[i].vertexindex = static_cast<int>( externalVertexIndex );
			externalVertexIndex += pmodel[i].numvertices * sizeof(mstudiovertex_t);

			// set expected base offsets to external data
			externalTangentsIndex = ( externalTangentsIndex + 3u ) & ~uintptr_t( 3u );
			pmodel[i].tangentsindex = static_cast<int>( externalTangentsIndex );
		...
		if ( !g_quiet )
		{
			printf("eyeballs   %7td bytes (%d eyeballs)\n", pData - cur, g_model[i]->numeyeballs );
		}
		...
		if( !g_quiet )
		{
			printf("flexes     %7td bytes (%d flexes)\n", pData - cur, g_numflexkeys );
		}
		cur = pData;
	}
...
void EnsureFileDirectoryExists( const char *pFilename )
{
	...
	*pLastSlash = 0;
	...
	std::error_code ec;
	std::filesystem::create_directories( std::filesystem::path( dirName ), ec );
}
...
#define ALIGN(b,s)		(((uintptr_t)(b)+(uintptr_t)(s)-1u)&~((uintptr_t)(s)-1u))

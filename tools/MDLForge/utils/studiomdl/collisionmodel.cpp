...
static bool g_hasCollisionModelBounds = false;
static Vector g_collisionModelMins;
static Vector g_collisionModelMaxs;

static bool VPhysicsDebugEnabled()
{
#ifdef _LINUX
	return getenv( "STUNSTICK_DEBUG_VPHYSICS" ) != NULL;
#else
	return false;
#endif
}

static int GetMaxConvexHullVertexCount()
{
	// Many Source vphysics builds have practical (and sometimes hard) limits on convex input vertex count.
	// 255 is a common limit due to internal 8-bit indexing.
	return 255;
}

static bool VectorPtrLessXYZ( const Vector *a, const Vector *b )
{
	if ( a->x != b->x )
		return a->x < b->x;
	if ( a->y != b->y )
...
static bool VectorPtrEqualXYZ( const Vector *a, const Vector *b )
{
	return a->x == b->x && a->y == b->y && a->z == b->z;
}

static CPhysConvex *ConvexFromVertsStable( Vector **verts, int vertCount )
{
	if ( !verts || vertCount <= 0 )
		return NULL;

	std::vector<Vector *> uniqueVerts;
	uniqueVerts.reserve( (size_t)vertCount );
	...
	std::sort(
		uniqueVerts.begin(),
		uniqueVerts.end(),
		[]( const Vector *a, const Vector *b ) { return VectorPtrLessXYZ( a, b ); } );

	uniqueVerts.erase(
		std::unique(
			uniqueVerts.begin(),
			uniqueVerts.end(),
			[]( const Vector *a, const Vector *b ) { return VectorPtrEqualXYZ( a, b ); } ),
		uniqueVerts.end() );

	const int maxVerts = GetMaxConvexHullVertexCount();
	if ( (int)uniqueVerts.size() > maxVerts )
	{
		if ( !g_quiet )
		{
			printf( "WARNING: collision hull has %d unique verts; truncating to %d\n", (int)uniqueVerts.size(), maxVerts );
		}

		std::vector<Vector *> sampled;
		sampled.reserve( (size_t)maxVerts );
		const size_t n = uniqueVerts.size();
		for ( int i = 0; i < maxVerts; i++ )
		{
			const size_t index = (size_t)i * n / (size_t)maxVerts;
			sampled.push_back( uniqueVerts[index] );
		}
		uniqueVerts.swap( sampled );
	}

	return physcollision->ConvexFromVerts( uniqueVerts.data(), (int)uniqueVerts.size() );
}
...
void CreateCollide( CPhysCollisionModel *pBase, CPhysConvex **pElements, int elementCount, const boundingvolume_t &bv )
{
	...
	if ( VPhysicsDebugEnabled() )
	{
		printf( "STUNSTICK_DEBUG_VPHYSICS: CreateCollide elementCount=%d pElements=%p\n", elementCount, (void *)pElements );
		for ( int e = 0; e < elementCount; e++ )
		{
			printf( "STUNSTICK_DEBUG_VPHYSICS: CreateCollide element[%d]=%p\n", e, (void *)pElements[e] );
		}
		fflush( stdout );
	}
	...
	for ( i = 0; i < elementCount; i++ )
	{
		if ( VPhysicsDebugEnabled() )
		{
			printf( "STUNSTICK_DEBUG_VPHYSICS: CreateCollide before ConvexVolume/SurfaceArea element[%d]=%p\n", i, (void *)pElements[i] );
			fflush( stdout );
		}
	...
	if ( VPhysicsDebugEnabled() )
	{
		printf( "STUNSTICK_DEBUG_VPHYSICS: CreateCollide before ConvertConvexToCollideParams\n" );
		fflush( stdout );
	}
	convertconvexparams_t params = {};
	params.Defaults();
	// Some shipped Source vphysics builds crash when serializing collision models unless the
	// optimized trace tables and drag areas are built.
	...
	if ( VPhysicsDebugEnabled() )
	{
		printf( "STUNSTICK_DEBUG_VPHYSICS: CreateCollide after ConvertConvexToCollideParams collide=%p\n", (void *)pBase->m_pCollisionData );
		fflush( stdout );
	}
...
//		printf("Bone %s has %d verts\n", joints.m_pModel->localBone[boneIndex].name, vertCount );
		...
		if ( vertCount )
		{
			CPhysConvex *pConvex = ConvexFromVertsStable( verts, vertCount );
...
int ProcessSingleBody( CJointedModel &joints )
{
	...
	g_hasCollisionModelBounds = true;
	g_collisionModelMins = boundingVolume.mins;
	g_collisionModelMaxs = boundingVolume.maxs;

	const bool forceSingleHull =
#ifdef _LINUX
		getenv( "STUNSTICK_FORCE_COLLISION_HULL" ) != NULL;
#else
		false;
#endif

	// Create the expensive "single hull for everything" convex lazily. Keeping an extra
	// convex alive alongside the per-mesh convex can trigger crashes in some shipped
	// vphysics.so builds.
	boundingVolume.pHull = NULL;

	if ( forceSingleHull )
	{
		boundingVolume.pHull = ConvexFromVertsStable( verts, pmodel->numvertices );
	...
	// loop through each bone and form a convex element
	for ( i = 0; i < pmodel->numvertices && !forceSingleHull; i++ )
	{
		...
		if ( vertCount > 2 )
		{
			...
			if ( IsApproximatelyPlanar( verts, vertCount, 0.5 ) )
			{
				...
				if ( !boundingVolume.pHull )
				{
					for ( int k = 0; k < pmodel->numvertices; k++ )
					{
						verts[k] = &worldVerts[k];
					}
					boundingVolume.pHull = ConvexFromVertsStable( verts, pmodel->numvertices );
					if ( boundingVolume.pHull )
					{
	...
	if ( elements.Size() )
	{
		...
		if ( VPhysicsDebugEnabled() )
		{
			printf( "STUNSTICK_DEBUG_VPHYSICS: numvertices=%d worldVerts=%p verts=%p vertID=%p elementsBase=%p boundingHull=%p\n",
				pmodel->numvertices,
				(void *)worldVerts,
				(void *)verts,
				(void *)vertID,
				(void *)elements.Base(),
				(void *)boundingVolume.pHull );

			for ( int e = 0; e < elements.Size(); e++ )
			{
				printf( "STUNSTICK_DEBUG_VPHYSICS: element[%d]=%p\n", e, (void *)elements[e] );
			}
			fflush( stdout );
		}
...
int DoCollisionModel( bool separateJoints )
{
	...
	const char *allow = getenv( "MDLFORGE_ALLOW_NATIVE_COLLISION" );
	if ( !allow || !allow[0] || allow[0] == '0' )
	{
		// Shipped vphysics builds on Linux/macOS are optimized for runtime and are not reliable for
		// collision model serialization (CollideSize/CollideWrite) needed to generate .phy.
		// Prefer running the Windows toolchain (studiomdl.exe) under Wine/Proton for $collisionmodel.
		Msg( "WARNING: Native $collisionmodel/$collisionjoints compilation is not supported on this platform. Use studiomdl.exe via Wine/Proton to generate .phy.\n" );
		Msg( "WARNING: Set MDLFORGE_ALLOW_NATIVE_COLLISION=1 to force-enable (experimental).\n" );
		return 0;
	}
#endif
	...
	if ( !physicsFactory )
	{
		Msg( "WARNING: Could not load vphysics; $collisionmodel/$collisionjoints compilation will be skipped.\n" );
		return 0;
	}

	physcollision = (IPhysicsCollision *)physicsFactory( VPHYSICS_COLLISION_INTERFACE_VERSION, NULL );
	physprops = (IPhysicsSurfaceProps *)physicsFactory( VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL );
	if ( !physcollision || !physprops )
	{
		Msg( "WARNING: Could not acquire required vphysics interfaces; $collisionmodel/$collisionjoints compilation will be skipped.\n" );
		return 0;
	}
...
void CollisionModel_ExpandBBox( Vector &mins, Vector &maxs )
{
	...
	if ( g_JointedModel.m_pCollisionList )
	{
		...
		// Prefer cached bounds derived from the collision SMD vertices. Some shipped
		// game vphysics builds can crash when calling CollideGetAABB() from tools.
		if ( g_hasCollisionModelBounds )
		{
			collideMins = g_collisionModelMins;
			collideMaxs = g_collisionModelMaxs;
		}
		else
		{
...
//-----------------------------------------------------------------------------
void CollisionModel_Write( int checkSum )
{
	if ( g_JointedModel.m_pCollisionList )
	{
...
//		}
		...
		if ( fp )
		{
			if ( VPhysicsDebugEnabled() && getenv( "STUNSTICK_DEBUG_VPHYSICS_TEST_BBOX" ) != NULL )
			{
				Vector mins( -1, -1, -1 );
				Vector maxs( 1, 1, 1 );
				CPhysCollide *pBBoxCollide = physcollision->BBoxToCollide( mins, maxs );
				printf( "STUNSTICK_DEBUG_VPHYSICS: BBoxToCollide=%p\n", (void *)pBBoxCollide );
				fflush( stdout );
				if ( pBBoxCollide )
				{
					int bboxSize = physcollision->CollideSize( pBBoxCollide );
					printf( "STUNSTICK_DEBUG_VPHYSICS: BBox CollideSize=%d\n", bboxSize );
					fflush( stdout );
					physcollision->DestroyCollide( pBBoxCollide );
				}
			}
			...
			int solidBinaryIndex = 0;
			pPhys = g_JointedModel.m_pCollisionList;
			while ( pPhys )
			{
				if ( !pPhys->m_pCollisionData )
				{
					Msg( "WARNING: Collision model data is null; .phy will not be generated.\n" );
					break;
				}

				if ( VPhysicsDebugEnabled() )
				{
					printf( "STUNSTICK_DEBUG_VPHYSICS: solid=%d before CollideSize collide=%p\n",
						solidBinaryIndex,
						(void *)pPhys->m_pCollisionData );
					fflush( stdout );
				}
				...
				if ( VPhysicsDebugEnabled() )
				{
					printf( "STUNSTICK_DEBUG_VPHYSICS: solid=%d collide=%p CollideSize=%d\n",
						solidBinaryIndex,
						(void *)pPhys->m_pCollisionData,
						size );
					fflush( stdout );
				...
				if ( size <= 0 )
				{
					Msg( "WARNING: Invalid collision model size (%d); .phy will not be generated.\n", size );
					break;
				}
				...
				if ( VPhysicsDebugEnabled() )
				{
					printf( "STUNSTICK_DEBUG_VPHYSICS: solid=%d CollideWrite buf=%p\n", solidBinaryIndex, (void *)buf );
					fflush( stdout );
				}

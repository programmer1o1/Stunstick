...
{
        // Always load the local stdio filesystem to avoid Steam/runtime path dependence.
        bSteam = false;
#ifdef _WIN32
        Q_strncpy( pFileSystemDLL, "filesystem_stdio.dll", nMaxLen );
#elif _LINUX
        Q_strncpy( pFileSystemDLL, "filesystem_i486.so", nMaxLen );
#else

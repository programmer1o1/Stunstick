...
{
	std::uint32_t fullWord = static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(handle));
	unsigned short serial = static_cast<unsigned short>( fullWord >> 16 );
	unsigned short index = static_cast<unsigned short>( fullWord & 0xFFFF );
	index--;
